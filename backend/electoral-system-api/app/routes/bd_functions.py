...
bd_functions_bp = Blueprint('bd_functions_bp', __name__)


# Rotas para a tabela Cargo

@bd_functions_bp.route('/limpar', methods=['POST'])
def limpar_banco():
    ...
    conn.commit()
    cur.close()
    return jsonify({"message": "Limpo com sucesso"}), 201


@bd_functions_bp.route('/alimentar', methods=['POST'])
def alimentar_banco():
    conn = get_db_connection()
    cur = conn.cursor()
    ...
    conn.commit()
    cur.close()
    return jsonify({"message": "Alimentado com sucesso"}), 201
